...
void GLApplication::updateCamera() {
    ...
    case Camera_Follow_Car: {
        ...
        _cameraStop.position(_car.orientation() * Vector3(0,3,10)  + _car.position());
    ...
    case Camera_Follow_Plane: {
        /*
        _camera.position(_airplane.position()+Vector3(0,1,-2));
        _camera.orientation(180,Vector3(0,1,0));
*/

        _cameraStop.position(_airplane.orientation() * Vector3(0,0,-5)  + _airplane.position());
        Quaternion quad = _airplane.orientation();
        quad.rotate(180,0,1,0);

        _cameraStop.orientation(quad);
    ...
    //interpolation -> animation entre la voiture et l'avion
