...
TreeBSP *ObjectBSP::consBSP(const vector<FaceBSP *> &tabFace) {
  ...
  if (tabFace.empty()) {
    ...
    //parcours des autres face
    for (int i = 1 ; i < tabFace.size() ; i++){
      //on sépare la face du pivot et on ajoute facePositive et/ou faceNegative aux listes respectives
...
void ObjectBSP::drawBSP(TreeBSP *tree,const Vector3 &eye) {
  ...
  //Affiche de la plus éloignée à la plus proche.
  //Donc left->node->right si node est SIGN_PLUS
  // ou  right->node->left si node est SIGN_MINUS
  if(tree == NULL) return;
  if(tree->node()->sign(eye) == SIGN_PLUS){
...
//  QFileInfo name=resourceFile(resourceName);
  ...
  for(unsigned int i=0; i<o->nbPosition(); i++) {
    VertexBSP *v=createVertex(o->positionOBJ(i));
    v->normal(o->normalOBJ(i)); // only for normal average
  }
  for(unsigned int i=0; i<o->nbFace(); i++) {
    FaceBSP *f=createFace();
    f->normal(o->normalFace(i));
    for(unsigned int j=0; j<o->nbVertex(i); j++) {
      f->add(_allVertex[o->positionIndex(i,j)]);
    }
  }
  delete o;
}
