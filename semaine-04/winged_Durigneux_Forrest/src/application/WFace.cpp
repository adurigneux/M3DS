...
void WFace::draw(bool withNormal) {
  ...
  do{
  //left
  //si la face est a gauche de notre point
  if(this == e->left()){
        end = e->end();
        position.push_back(end->position());
        normal.push_back(end->normal());
        e = e->succLeft();
  }else {
      //right
        begin = e->begin();
        position.push_back(begin->position());
        normal.push_back(begin->normal());
        e = e->succRight();
  }
...
void WFace::drawLineCatmull() {
  WEdge *e=this->edge();
  do {
    vector<Vector3> vector = { this->pointFace(),e->pointEdge() };
    p3d::drawLines( vector ,-1) ;
    if (this==e->left())
      e=e->succLeft();
    else
      e=e->succRight();
