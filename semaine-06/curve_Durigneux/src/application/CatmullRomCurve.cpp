...
void CatmullRomCurve::draw() {
    ...
    //(affectez les points avec le setter cubic.point(j,un_point), puis de la tracer
    //(en appelant simplement cubic.drawBezier() ).
...
void CatmullRomCurve::setup() {
    ...
    // A COMPLETER : il s'agit d'initialiser les points intermédiaires
    //(les points verts) en appelant les setters intermediate(i,0,<un Vector3>) et intermediate(i,1,<un Vector3>) )
    // les points intermédiaires doivent être fixés par la méthode vue en cours
    //(tangentes parallèles aux segments [point(i-1),point(i+1)]).

    float alpha = 0.4;
    for (int i = 1; i < nbPoint() -1; i++) {
        parallele = alpha * (point(i+1) - point(i-1));
        this->intermediate(i,0,point(i) + parallele);
        this->intermediate(i-1,1,point(i) - parallele);
    }
