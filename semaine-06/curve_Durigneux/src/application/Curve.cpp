...
Vector3 Curve::evalBezier(double t) {
    ...
    if (nbPoint()>1) {
        ...
        int castelSize = castel.size();
        // A COMPLETER : appliquer la méthode de De Casteljau
        for (int i = 1; i < castelSize; i++) {
            int nbOperations = castelSize - i;
...
Vector3 Curve::evalCubicVelocity(double t) {
    ...
    res=3*t*t*(-point(0)+3*point(1)-3*point(2)+point(3))+
            2*t*(3*point(0)-6*point(1)+3*point(2))+
            (-3*point(0)+3*point(1));
...
Vector3 Curve::evalCubicAcceleration(double t) {
    ...
    res=6*t*(-point(0)+3*point(1)-3*point(2)+point(3))+
            2*(3*point(0)-6*point(1)+3*point(2));
