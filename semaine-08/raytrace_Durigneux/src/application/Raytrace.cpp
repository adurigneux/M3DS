...
Vector3 Raytrace::computeLocalColor(const Intersection &intersection) {
  ...
  */
    ...
    int nombrelumiere = _scene->nbLight();

    for(int i = 0; i< nombrelumiere; i++){
        ...
        if (nearestIntersection!=NULL) {
            double lambda =  nearestIntersection->lambda();
            if (lambda > 1) {
