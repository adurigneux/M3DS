...
void Engine::euler(double dt) {
    for(unsigned int i=0; i<_particleList->size(); i++) {
      ...
      */
            ...
            p->velocity(velocity + ((force/mass)*dt));
            p->position(position + (velocity*dt));
...
void Engine::computeForce() {
    for(unsigned int i=0; i<_particleList->size(); i++) {
        Particle *p=(*_particleList)[i];
        if (p->alive()) {
            Vector3 forceToAdd= Vector3(0,-9.81,0);
            p->addForce(forceToAdd*p->mass());
        }
    }
}
        ...
        for(unsigned int i=0; i<_particleList->size(); i++) {
            ...
            if (p->alive()) {
                ...
                if((p->position() - Vector3(0,p->radius(),0) - plane->point()).dot(plane->normal()) <= 0) {
                    ...
                    posCorrection = (1.0+epsilon) * (H - (p->position() - Vector3(0,p->radius(),0)));
                    velCorrection = - (1.0+epsilon) * ((p->velocity().dot(plane->normal())) * plane->normal()) ;
            ...
            for(unsigned int j=i+1; j<_particleList->size(); j++) {
                ...
                if (p2->alive()) {
                    ...
                    if( d < 0 ){
                        ...
                        //voir fonction slide 29 des slides d'animations
                        velCorrectionP1 = - (computeImpulse(p1,p2,n,epsilon)/p1->mass())*n;
                        velCorrectionP2 = (computeImpulse(p1,p2,n,epsilon)/p2->mass())*n;


                        //voir fonction slide 30 des slides d'animations
                        posCorrectionP1 = (1.0 + epsilon) * (p2->mass() / (p1->mass() + p2->mass())) * d * n;
                        posCorrectionP2 = - (1.0 + epsilon) * (p1->mass() / (p1->mass() + p2->mass())) * d * n;
