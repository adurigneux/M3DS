...
void EngineBox::computeForce() {
    ...
    if(_cursorActive == true){
        Box *box=_boxList->selected();
        Vector3 l=_cursor - box->attachWorld();
        box->addForce(l * 1.5);
        box->addMoment(box->force().cross(box->position() - box->attachWorld()));
    }
    }
...
void EngineBox::euler(double dt) {
    for(unsigned int i=0; i<_boxList->size(); i++) {
        ...
        double vTheta = b->omega().z();
        b->theta(b->theta()+ (vTheta*dt));
