...
p3d::Vector3 Camera2::windowToNDC(int x,int y) {
    double xx = (x - _viewWidth/2)*2-1;
    double yy = (y - _viewHeight/2)*2-1;

    Vector3 res(xx/_viewWidth,yy/_viewHeight,-1);
    //cout << "W : " << _viewX << " " << _viewY << " "<< _viewWidth << " " << _viewHeight << endl;

     // cout << "window to NDC : " << res <<" "<< x << " " << y << endl;
...
p3d::Line Camera2::pickingRay(int x, int y) {
    ...
    // Window to world system
    Vector3 win2World = windowToWorld(x,y);

    res.direction(win2World-res.point());
